...
function goToTestPage(id) {
  window.location.href = "{{ route('companystatus') }}"+id;
                ...
                $ggrtax=0;
				@endphp
                    @foreach($bcompanies as $bookmarker)
                    @php 
                $total_tickets_sold +=$bookmarker->total_tickets_sold ;
				 $sales +=$bookmarker->publicLotterycompany->sum('sales')  ;
				 $payouts +=$bookmarker->publicLotterycompany->sum('payouts')  ;
				 $wht +=$bookmarker->wht ;
				 $ggr +=$bookmarker->publicLotterycompany->sum('ggr') ;
                ...
                <tr>
                        ...
                        <td>{{ $bookmarker->publicLotterycompany->sum('sales') }}</td>
                        <td>{{ $bookmarker->publicLotterycompany->sum('payouts') }}</td>
                        <td>{{ $bookmarker->publicLotterycompany->sum('ggr') }}</td>
